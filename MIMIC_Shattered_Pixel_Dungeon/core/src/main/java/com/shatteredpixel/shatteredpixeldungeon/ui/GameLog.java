...
public class GameLog extends Component implements Signal.Listener<String> {
	...
	public synchronized void update() {
		...
		for (String text : textsToAdd){
			...
			if (text.startsWith( GLog.CONNECTION )) {
				text = text.substring( GLog.CONNECTION.length() );
				color = CharSprite.CONNECTION;
			} else
			if (text.startsWith( GLog.AGENT_ERROR )) {
				text = text.substring( GLog.AGENT_ERROR.length() );
				color = CharSprite.AGENT_ERROR;
			}
