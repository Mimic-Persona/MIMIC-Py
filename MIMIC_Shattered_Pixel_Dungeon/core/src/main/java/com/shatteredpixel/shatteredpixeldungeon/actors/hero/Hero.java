...
public class Hero extends Char {
	...
	// Upgrade is special since it does not update the game
	public void upgradeTalent( Talent talent ){
		Dungeon.actionCounter++;

		JSONObject prevAction = new JSONObject();
		prevAction.put( "action", "Upgrade" );
		prevAction.put( "obj1", talent.title() );
	...
	private boolean actMove( HeroAction.Move action ) {
		...
		//Hero moves in place if there is grass to trample
			...
			GLog.w("Acted to: " + Arrays.toString(Position.transferPos2XY(action.dst)) + "and trampled grass.");
			return false;
	...
	private boolean actInteract( HeroAction.Interact action ) {
		...
		if (ch.isAlive() && ch.canInteract(this)) {
			...
			GLog.h("Interacting with " + ch.name() + "at: " + Arrays.toString(Position.transferPos2XY(ch.pos)));
			...
			if (fieldOfView[ch.pos] && getCloser( ch.pos )) {
				...
				GLog.e("Cannot interact with " + ch.name() + "at: " + Arrays.toString(Position.transferPos2XY(ch.pos)));
				ready();
				return false;
			}
	...
	private boolean actBuy( HeroAction.Buy action ) {
		...
		if (pos == dst) {
			...
			GLog.h("Buying at: " + Arrays.toString(Position.transferPos2XY(dst)));
			...
			GLog.e("Cannot buy at: " + Arrays.toString(Position.transferPos2XY(dst)));
			ready();
			return false;
		}
	}
	...
	private boolean actAlchemy( HeroAction.Alchemy action ) {
		...
		if (Dungeon.level.distance(dst, pos) <= 1) {
			...
			GLog.h("Alchemizing at: " + Arrays.toString(Position.transferPos2XY(dst)));
			ready();
			...
			GLog.e("Cannot alchemy at: " + Arrays.toString(Position.transferPos2XY(dst)));
			ready();
			return false;
		}
	}
	...
	private boolean actPickUp( HeroAction.PickUp action ) {
		...
		if (pos == dst) {
			...
			if (heap != null) {
				...
				if (item.doPickUp( this )) {
					...
					if (item instanceof Dewdrop
						...
						GLog.p("You picked up " + item.name() + ".");
					...
					if (waitOrPickup) {
						GLog.w("You failed to pick up the item and waited for 1 turn.");
					...
					if (item instanceof Dewdrop
							...
							|| item instanceof DriedRose.Petal
							|| item instanceof Key) {
						GLog.p("You cannot pick up " + item.name() + ".");
					} else {
						GLog.newLine();
				...
				GLog.e("No item to pick up at: " + Arrays.toString(Position.transferPos2XY(dst)));
				ready();
			}
			...
			GLog.e("Cannot pick up at: " + Arrays.toString(Position.transferPos2XY(dst)));
			ready();
			return false;
		}
	}
		...
		if (Dungeon.level.adjacent( pos, dst ) || pos == dst) {
			...
			if (heap != null && (heap.type != Type.HEAP && heap.type != Type.FOR_SALE)) {
				...
				case TOMB:
					...
					GLog.p("You opened the tomb at: " + Arrays.toString(Position.transferPos2XY(dst)));
					break;
				case SKELETON:
					GLog.p("You opened the skeleton at: " + Arrays.toString(Position.transferPos2XY(dst)));
					break;
				case REMAINS:
					GLog.p("You opened the remains at: " + Arrays.toString(Position.transferPos2XY(dst)));
					break;
				default:
					GLog.p("You opened the chest at: " + Arrays.toString(Position.transferPos2XY(dst)));
				...
				GLog.e("Nothing to open at: " + Arrays.toString(Position.transferPos2XY(dst)));
				ready();
			}
			...
			GLog.e("Cannot open at: " + Arrays.toString(Position.transferPos2XY(dst)));
			ready();
			return false;
		}
	}
	...
	private boolean actUnlock( HeroAction.Unlock action ) {
		...
		if (Dungeon.level.adjacent( pos, doorCell )) {
			...
			if (hasKey) {
				...
				GLog.p("You unlocked the door at: " + Arrays.toString(Position.transferPos2XY(doorCell)));
				...
				GLog.e( Messages.get(this, "locked_door") );
				ready();
			}
			...
			GLog.e("Cannot unlock at: " + Arrays.toString(Position.transferPos2XY(doorCell)));
			ready();
			return false;
		}
	}
	...
	public boolean actMine(HeroAction.Mine action){
		if (Dungeon.level.adjacent(pos, action.dst)){
			...
			if ((Dungeon.level.map[action.dst] == Terrain.WALL
				...
				GLog.w("Mining at: " + Arrays.toString(Position.transferPos2XY(action.dst)));
			} else {
				GLog.e("Cannot mine at: " + Arrays.toString(Position.transferPos2XY(action.dst)));
				ready();
			}
			return false;
	...
	private boolean actTransition(HeroAction.LvlTransition action ) {
		...
		if (rooted) {
			...
			ready();
			GLog.e("You are rooted to the ground, try to get rid of it!");
			return false;
			...
			GLog.e("Cannot transit at: " + Arrays.toString(Position.transferPos2XY(stairs)));
			ready();
			return false;
		}
	}
	...
	private boolean getCloser( final int target ) {
		...
		if (Dungeon.level.adjacent( pos, target )) {
			...
			if (Actor.findChar( target ) == null) {
				...
				if (walkingToVisibleTrapInFog
					...
					GLog.e("You can't walk into a visible trap in fog!");
					return false;
				}
			}
			...
			if (path == null) {
				GLog.e("There is no path to: " + Arrays.toString(Position.transferPos2XY(target)) + ", change another action.");
				return false;
			}
		...
		if (step != -1) {
			...
			if (Dungeon.level.pit[step] && !Dungeon.level.solid[step]
				...
				// Remove double check for jump confirmation
			...
			GLog.w("Moved to: " + Arrays.toString(Position.transferPos2XY(step)));
	...
	public boolean handle( int cell ) {
		...
		if (cell >= Dungeon.level.length() || cell < 0) {
			GLog.e("There is no such tile in this level.");
			return false;
		}
