...
public abstract class Mob extends Char {
	...
	public void yell( String str ) {
		GLog.newLine();
		GLog.n( Messages.titleCase(name()) + ": \"" + str + "\""  );
	}
	...
	public String toString() {
		int x = Position.transferPos2XY(pos)[0];
		int y = Position.transferPos2XY(pos)[1];
		int heroX = Position.transferPos2XY(Dungeon.hero.pos)[0];
		int heroY = Position.transferPos2XY(Dungeon.hero.pos)[1];

		String res = name().replace(" ", "_").toLowerCase();
		res += "(" + HP + "/" + HT + ", " + state.getTag() + ", " + Arrays.toString(Position.transferPos2XY(pos));

		if (enemy instanceof Hero && state != SLEEPING && (Math.abs(x - heroX) + Math.abs(y - heroY)) <= 2){
			res += ", inRange";
		}
