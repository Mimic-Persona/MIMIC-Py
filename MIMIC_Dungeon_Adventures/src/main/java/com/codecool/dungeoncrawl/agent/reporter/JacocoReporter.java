...
import static com.codecool.dungeoncrawl.agent.reporter.JacocoCoverageFetcher.fetchTotalCoverage;
import static com.codecool.dungeoncrawl.agent.reporter.JacocoReportGenerator.generateJacocoHtmlReport;
...
public class JacocoReporter {
    ...
    private static final String OUT_DIR = "F:/Generative_Agents/SPD-DATA/jacoco-reports-html/" + ReporterClient.PREFIX + "/" + ReporterClient.PREFIX;
    ...
    public static int report(int rowNum) throws IOException, JSONException {
        ...
        if (covDataDiff != null) {
            for (Iterator it = covDataDiff.keys(); it.hasNext(); ) {
                ...
                if (covDataDiff.getDouble(key) != 0) {
    ...
    private static String generateCoverageReport() throws IOException {
        ...
        try {
            packageName = generateJacocoHtmlReport(CLASS_FILES_DIR, EXEC_FILES_DIR, SOURCE_FILES_DIR, OUT_DIR + "-" + timestamp);
     ...
     * @throws IOException if an I/O error occurs
     */
    public static String dumpDataWithoutCumulating() throws IOException {
    ...
    public static String dumpData() throws IOException {
        ExecDumpClient client = new ExecDumpClient();
