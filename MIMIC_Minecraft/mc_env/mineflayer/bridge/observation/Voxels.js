...
function getSurroundingBlocks(bot, x_distance, y_distance, z_distance) {
    ...
    let observerEntity;
    if (bot.is_player_observer) {
        const playerFilter = (entity) => entity.type === 'player'
        observerEntity = bot.nearestEntity(playerFilter)
    } else {
        observerEntity = bot.entity;
    }

    for (let x = -x_distance; x <= x_distance; x++) {
        for (let y = -y_distance; y <= y_distance; y++) {
            for (let z = -z_distance; z <= z_distance; z++) {
                const block = bot.blockAt(observerEntity.position.offset(x, y, z));
