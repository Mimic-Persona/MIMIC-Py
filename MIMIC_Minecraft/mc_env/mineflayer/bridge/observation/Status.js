...
class Status extends Observation {
    ...
    observe() {
        ...
        if (this.bot.is_player_observer) {
            ...
            return {
                ...
                position: observerEntity.position,
                velocity: this.bot.entity.velocity,
                yaw: this.bot.entity.yaw,
                pitch: this.bot.entity.pitch,
                onGround: observerEntity.onGround,
                equipment: this.getEquipment(),
                name: observerEntity.name,
                ...
                biome: this.bot.blockAt(observerEntity.position)
                    ? this.bot.blockAt(observerEntity.position).biome.name
                    : "None",
                entities: this.getEntities(observerEntity),
        ...
        return {
            ...
            biome: this.bot.blockAt(this.bot.entity.position)
                ...
                : "None",
            entities: this.getEntities(this.bot.entity),
            timeOfDay: this.getTime(),
            inventoryUsed: this.bot.inventoryUsed(),
            elapsedTime: this.bot.globalTickCounter,
        };
    ...
    getEntities(observerEntity) {
        ...
        for (const id in entities) {
            ...
            if (entity.position.distanceTo(observerEntity.position) < 32) {
                if (!mobs[entity.name]) {
